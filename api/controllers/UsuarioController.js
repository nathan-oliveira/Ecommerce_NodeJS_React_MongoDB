...
class UsuarioController {
  ...
  showCompleteRecovery(req, res, next) {
    ...
    Usuario.findOne({"recovery.token": req.query.token })
      .then(usuario => {
        ...
        if(new Date(usuario.recovery < new Date())) return res.render("recovery", { error: "Token expirado. Tente novamente.", success: null });
  ...
  completeRecovery(req, res, next) {
    ...
    Usuario.findOne({ "recovery.token": token })
      .then(usuario => {
        if(!usuario) return res.render("recovery", { error: "Usuário não identificado", success: true })
